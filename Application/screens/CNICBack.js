...
const CNICBack = ({ navigation }) => {
  ...
  const handleUpload = (image) => {
    ...
    fetch("https://api.cloudinary.com/v1_1/omarfarooqkhan/image/upload", {
      ...
      .then((res) => res.json())
      .then((data) => {
        vendor.cnicBack = data.url;
        dispatch({ type: "ADD_NEW_VENDOR", payload: vendor });
        Alert.alert("Success", "CNIC Back-side uploaded successfully!");
  ...
  return (
    <View style={{ height: "100%", backgroundColor: Colors.secondary }}>
      ...
      >
        ...
        <Image
          source={require("../assets/images/CNIC_back_sample.jpg")}
          style={{
            borderWidth: 1,
            borderColor: "grey",
            marginVertical: hp("5%"),
